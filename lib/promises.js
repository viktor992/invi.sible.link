...
function retrieve(daysago, target, type) {
    ...
    var max = moment().startOf('day').add(1, 'd');
    ...
    //  var filter = { "start": { "$gte": new Date( new Date( min.format() ).toISOString()), 
    //                            "$lte": new Date( new Date( max.format() ).toISOString()) }};
...
function manageOptions() {
    ...
    if(!found && _.keys(nconf.get('campaigns')).indexOf(tname) !== -1) {
        ...
        //machetils.fatalError("campaign not found", tname);
    }
